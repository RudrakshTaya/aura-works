    ...
    <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/80 border-b">
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center gap-4">
          ...
          >
            <span className="inline-block rounded-md bg-primary/70 px-2 py-1 mr-2 shadow-sm">
              Artistry
            </span>
          ...
          <form
            onSubmit={submit}
            className="ml-auto flex-1 max-w-md hidden md:flex"
          >
            <div className="relative w-full">
              ...
              <button
                aria-label="Search"
                className="absolute right-1 top-1/2 -translate-y-1/2 rounded-full px-3 py-1.5 bg-primary/70 text-foreground/90"
              >
                Go
              </button>
            </div>
      ...
      <div className="container mx-auto px-4 py-10 grid gap-6 md:grid-cols-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-sm">
            Discover unique handmade art, crafts, paintings, and custom creative
    ...
    <div className="flex items-center gap-3 ml-2 relative">
      <Link to="/cart" className="relative inline-flex items-center text-sm">
        ...
        {count > 0 && (
          <span
            ...
            className="ml-1 inline-flex items-center justify-center min-w-[1.25rem] h-5 rounded-full bg-foreground text-background text-[10px] px-1.5"
          >
            {count}
          </span>
        )}
      ...
      {user ? (
        <div className="flex items-center gap-2 relative">
          <button
            onClick={() => setOpen((s) => !s)}
            className="inline-flex items-center gap-2 text-sm rounded-md px-2 py-1 hover:bg-muted/50"
          >
            {user.avatar ? (
              <img
                src={user.avatar}
              ...
              <ul className="p-2 text-sm">
                <li>
                  <Link
                    ...
                    className="block px-2 py-2 hover:bg-muted/60 rounded"
                  >
                    Orders
                  </Link>
                </li>
                ...
                <li>
                  <button
                    ...
                    className="w-full text-left px-2 py-2 hover:bg-muted/60 rounded"
                  >
                    Logout
                  </button>
                </li>
