...
export default function Index() {
  ...
  const { data: sellers = [] } = useQuery({ queryKey: ["sellers"], queryFn: getFeaturedSellers });
console.log(products);
  const featured = products.slice(0, 8);
  const trending = [...products].sort((a,b)=> (b.rating?.count ?? 0) - (a.rating?.count ?? 0)).slice(0,8);
      ...
      <section className="container mx-auto px-4 pt-10 pb-16 md:pt-16">
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <p className="mt-4 text-muted-foreground max-w-prose">Shop unique handmade art, crafts, paintings, and custom creative products from local sellers. Support artists near you with every purchase.</p>
            ...
            <div className="mt-6 flex items-center gap-3">
              <Link to="/products" className="rounded-full bg-foreground text-background px-5 py-2.5 text-sm font-medium">Explore Products</Link>
              <Link to="/wishlist" className="text-sm underline underline-offset-4">View Wishlist</Link>
            </div>
          </div>
