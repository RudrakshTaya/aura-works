    ...
    <div className="container mx-auto px-4 py-10 grid gap-8 md:grid-cols-2">
      ...
      <div>
        ...
        <div className="mt-6 flex items-center gap-3">
          <button onClick={handleAdd} disabled={adding} className="rounded-full bg-foreground text-background px-5 py-2.5 text-sm font-medium">{adding ? "Adding…" : "Add to Cart"}</button>
          <button onClick={handleToggleWish} className={`rounded-full px-5 py-2.5 text-sm font-medium ${wished ? "bg-destructive text-background" : "bg-secondary"}`}>{wished ? "♥ Wishlisted" : "Wishlist"}</button>
        </div>
      </div>
    </div>
  );
